...
namespace hero_chassis_controller{
    ...
    void HeroChassisController::update(const ros::Time& time, const ros::Duration& period){
        ...
        double back_right_commanded_effort = pid_back_right_controller_.computeCommand(back_right_error,period);
            double now_power_ = (fabs(front_right_commanded_effort)+fabs(front_left_commanded_effort)+fabs(back_right_commanded_effort)+fabs(back_left_commanded_effort))*joint_torque_[0]*4+k1_*(joint_torque_[0]*4)*(joint_torque_[0]*4)+k2_*(fabs(front_right_commanded_effort)+fabs(front_left_commanded_effort)+fabs(back_right_commanded_effort)+fabs(back_left_commanded_effort))*(fabs(front_right_commanded_effort)+fabs(front_left_commanded_effort)+fabs(back_right_commanded_effort)+fabs(back_left_commanded_effort));
            if(now_power_>max_power_){
    ...
    void HeroChassisController::cmdCallback(const geometry_msgs::Twist::ConstPtr &msg) {
        ...
        vel_.linear.y=msg->linear.y;
        vel_.angular.z=msg->angular.z;
        double a = vel_.linear.x;
        double b = vel_.linear.y;
        if(speed_mode_=="global"){
        try{
           tf::TransformListener tf_listener_;
            geometry_msgs::Vector3Stamped vel_local_,vel_global_;
            vel_global_.header.frame_id="odom";
            vel_global_.vector.x = a;
            vel_global_.vector.y = b;
            vel_global_.vector.z = 0.0;
 //          ros::Time now = ros::Time::now();
            tf_listener_.waitForTransform("base_link","odom",ros::Time(),ros::Duration(5.0));
            tf_listener_.transformVector("base_link",vel_global_,vel_local_);
            vel_.linear.x=vel_local_.vector.x;
            vel_.linear.y=vel_local_.vector.y;        
        ...
        front_left_vel_ = vel_.linear.x-vel_.linear.y-vel_.angular.z*((wheel_track_+wheel_base_)/2.0);
        front_right_vel_ = vel_.linear.x+vel_.linear.y+vel_.angular.z*((wheel_track_+wheel_base_)/2.0);
        back_right_vel_ = vel_.linear.x-vel_.linear.y+vel_.angular.z*((wheel_track_+wheel_base_)/2.0);
        back_left_vel_ = vel_.linear.x+vel_.linear.y-vel_.angular.z*((wheel_track_+wheel_base_)/2.0);
    ...
    void HeroChassisController::odom_update(const ros::Time& time, const ros::Duration& period){
        ...
        double delta_x_ = (odom_vx_* cos(odom_th_)-odom_vy_ * sin(odom_th_))*dt_;
        double delta_y_ = (odom_vx_*sin(odom_th_)+odom_vy_*cos(odom_th_))*dt_;
        double delta_th_ = odom_vth_*dt_;
        odom_x_ += delta_x_;
        odom_y_ += delta_y_;
        odom_th_ += delta_th_;
        geometry_msgs::Quaternion odom_quat_ = tf::createQuaternionMsgFromYaw(odom_th_);
